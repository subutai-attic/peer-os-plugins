 ...
 */
class OozieSetupStrategy implements ClusterSetupStrategy
{
    //    private Environment environment;
    final OozieClusterConfig config;
    final OozieImpl manager;
    ...
    private void configure() throws ClusterSetupException
    {
        ...
        for ( ContainerHost node : environment.getContainerHostsByIds( config.getNodes() ) )
        {
            try
            {
                node.execute( new RequestBuilder( Commands.make(CommandType.INSTALL_CLIENT) ).withTimeout( 600 ) );
            }
            catch ( CommandException e )
            {
                throw new ClusterSetupException(
