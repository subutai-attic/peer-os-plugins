...
public class MongoAlertListener extends ExceededQuotaAlertHandler
{
			...
			throws AlertHandlerException
    {
        ...
        {
			LOGGER.info                                         ( String.format( "Alert source host %s does not belong "
							+ "to Mongo cluster",
...
//                    int ramQuota = mongo.getQuotaManager().getRamQuota( sourceHost.getId() );
			...
			// add new node
			MongoClusterConfig mongoClusterConfig =
					mongo.getCluster( targetCluster.getClusterName() );
			if ( mongoClusterConfig == null )
			{
				throwAlertException(
						String.format( "Mongo cluster %s not found", targetCluster.getClusterName() ), null );
				return;
			}
