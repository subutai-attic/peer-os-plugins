...
/**
 * sample command : solr:check-node test \ {cluster name} solr \ { container hostname }
 */
@Command( scope = "solr", name = "check-node", description = "Command to check Solr service" )
public class StatusServiceCommand extends OsgiCommandSupport
{
    ...
    {
        UUID uuid = getSolrManager().checkNode( clusterName, hostname );
