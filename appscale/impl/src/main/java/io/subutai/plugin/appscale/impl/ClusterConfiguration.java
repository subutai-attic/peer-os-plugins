...
public class ClusterConfiguration implements ClusterConfigurationInterface
{
    ...
    private void createRunSH( EnvironmentContainerHost containerHost, AppScaleConfig config )
    {

        try
        {
            containerHost.execute( new RequestBuilder( "echo '" + returnRunSH( config ) + "' > /root/run.sh" ) );
