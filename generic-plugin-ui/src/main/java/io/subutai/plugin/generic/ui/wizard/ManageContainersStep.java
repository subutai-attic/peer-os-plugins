...
public class ManageContainersStep extends Panel
{
	...
	private void populateTable (final Table containerTable, final Set<EnvironmentContainerHost> hosts, final Profile currentProfile )
	{
		...
		for (final ContainerHost host : hosts)
		{
			...
			executeBtn.addClickListener (new Button.ClickListener()
			{
				...
				public void buttonClick (final Button.ClickEvent clickEvent)
				{
					addOutput (genericPlugin.executeCommandOnContainer (host, (Operation) operationSelect.getValue()));
				}
			});
	...
	private void addClickListenerToExecuteButton (final Button executeBtn, final ContainerHost host, final Operation operation)
	{
		executeBtn.addClickListener (new Button.ClickListener()
		{
			@Override
